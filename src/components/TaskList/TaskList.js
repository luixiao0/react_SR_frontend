...
function DummyTaskcard(props) {
  return (
    <>
      <div key={props.id} className={`${props.dark ?'bg-gray-900': 'bg-white' } duration-100 rounded-xl py-4 text-center select-none`} >
  ...
  card_dummy(DL) {
    ...
    if (this.state.files.length) {
      return (this.state.files.map((obj) => {
        // console.log(obj)
        return (<Taskcard key={obj.id} id={obj.id} state={obj.s} params={obj.p} date={obj.date} name={obj.name} DL={DL} dark={this.props.card}/>);
      }))
    }
    else {
      return (<DummyTaskcard dark={this.props.dark}/>)
    }
  }
  ...
  render() {
    ...
    return (
      <div className="w-full">
        <div className={`${this.props.dark ?'bg-gray-700': 'bg-white' } shadow-sm rounded-xl mt-2 p-1 mx-2 duration-100`}>
