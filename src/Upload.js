    ...
    return (
      <div className="w-full px-2 mt-2">
        <Tab.Group
          ...
          <Tab.List className='flex p-1 rounded-xl dark:bg-gray-800 bg-white duration-100 shadow-sm space-x-1 bg-opacity-60 '>
            {Object.keys(this.categories).map((category) => {
              return (
                <Tab
                  ...
                  key={category}
                  className={({ selected }) => {
                    let style = 'text-black-700 dark:hover:bg-gray-700 hover:bg-gray-200'
                    if (selected) {
                      style = 'dark:bg-gray-900 bg-white hover:bg-white/[0.90] hover:text-blue-700 shadow-md'
                    }
            ...
            {Object.values(this.categories).map((posts) => (
              <Tab.Panel
                ...
                className={this.classNames(
                  'dark:bg-gray-800 bg-white shadow-sm rounded-xl p-3 ring-offset-2 ring-offset-blue-400 ring-white ring-opacity-60'
                )}
              >
                {this.params(posts)}
              </Tab.Panel>
